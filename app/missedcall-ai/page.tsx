...
import {
  Phone, MessageSquareText, CalendarCheck, ShieldBan, Globe, Send, BarChart3,
  Clock, Shield, DollarSign, ArrowRight, ArrowUpRight, Check, PhoneMissed,
...
function Typing({ side }: { side: 'in' | 'out' }) {
  return (
    <div className={`flex ${side === 'out' ? 'justify-end' : 'justify-start'}`}>
      <div className="flex items-center gap-1 px-3.5 py-3" style={{ background: side === 'out' ? 'rgba(238,107,26,0.85)' : 'rgba(242,240,235,0.9)', borderRadius: 12 }}>
  ...
  if (bubble.kind === 'system') {
    return (
      <div className="aa-rise my-1 text-center">
        <span className="font-mono text-[9.5px] uppercase tracking-[0.18em]" style={{ color: '#6E7681' }}>{bubble.text}</span>
      </div>
    )
  }
    ...
    return (
      <div className="aa-rise mt-2 flex flex-wrap items-center justify-center gap-1.5 border-t-2 pt-3" style={{ borderColor: 'rgba(110,118,129,0.3)' }}>
        {bubble.chips.map((c, i) => (
          <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 font-mono text-[9.5px] font-bold uppercase tracking-wider" style={{ background: 'rgba(238,107,26,0.14)', color: '#EE6B1A' }}>
...
function SmsThread() {
  ...
  return (
    <div ref={ref} className="aa-float relative mx-auto w-full max-w-[380px]">
      <div className="relative overflow-hidden border-2" style={{ borderColor: '#16181C', background: '#16181C', boxShadow: '0 0 0 1px rgba(110,118,129,0.4)' }}>
        {/* Header */}
        <div className="flex items-center justify-between border-b-2 px-4 py-3" style={{ borderColor: 'rgba(110,118,129,0.35)', background: '#1A4A70' }}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              ...
              <div className="font-mono text-[9px] uppercase tracking-[0.2em]" style={{ color: 'rgba(242,240,235,0.7)' }}>Auto text-back · live</div>
            </div>
          </div>
          <span className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest" style={{ color: '#FFFFFF' }}>
            <span className="aa-live-dot inline-block h-2 w-2 rounded-full" style={{ background: '#EE6B1A' }} />on
          </span>
        </div>
        {/* Thread */}
        <div className="flex min-h-[460px] flex-col gap-2.5 px-4 py-4" style={{ background: '#16181C' }}>
          {THREAD.slice(0, count).map((b, i) => <BubbleEl key={i} bubble={b} />)}
      ...
      <div className="absolute -right-3 -top-3 z-10 hidden items-center gap-2 border-2 px-3 py-2 sm:flex" style={{ borderColor: '#16181C', background: '#EE6B1A' }}>
        <PhoneMissed size={14} strokeWidth={2.5} style={{ color: '#16181C' }} />
        <span className="font-mono text-[10px] font-bold uppercase tracking-widest" style={{ color: '#16181C' }}>Lead saved</span>
      </div>
    </div>
  )
}
...
function FAQItem({ question, answer }: { question: string; answer: string }) {
  return (
    <details
      ...
      style={{ borderColor: 'rgba(110,118,129,0.3)', background: 'rgba(242,240,235,0.02)' }}
    >
    ...
    <div className="min-h-dvh w-full overflow-x-hidden" style={{ background: '#16181C', color: '#F2F0EB' }}>
      ...
      <section className="border-b-2" style={{ borderColor: 'rgba(110,118,129,0.3)' }}>
        <div className="mx-auto max-w-7xl px-5 sm:px-8 py-12">
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.24em] text-center mb-8" style={{ color: '#6E7681' }}>The math doesn&apos;t lie</p>
