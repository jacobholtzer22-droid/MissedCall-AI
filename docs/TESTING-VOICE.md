...
- Two phones (or one phone + a friend): one to **call** the Telnyx number, one as the **business owner** (forwarding number)
- [ngrok](https://ngrok.com/) installed (so Telnyx can reach your local server)
...
This URL is used to build the **voice-gather** and **voice-dial-status** callback URLs Telnyx will call.

## 3. Configure Telnyx webhook

1. Go to [Telnyx Portal → Numbers → Manage](https://portal.telnyx.com).
2. Click your Telnyx phone number.
3. Under **Voice**, set:
   - **Inbound Webhook URL**: `https://YOUR-NGROK-URL.ngrok-free.app/api/webhooks/voice`
   - **HTTP**: POST
4. Save.
...
1. The business must use **this same Telnyx number** as its "Telnyx phone number".
...
Ensure the business record in the DB has `callScreenerEnabled: true` and a valid `forwardingNumber` and `telnyxPhoneNumber`.

## 5. Test scenarios

### A. Owner answers (no SMS)

1. From **Phone A**, call your **Telnyx number**.
...
4. After the dial times out, Telnyx will call your **voice-dial-status** webhook.
...
`📵 Owner did not answer (status: no-answer ), triggering SMS` and `📤 Sent MissedCall AI SMS after missed dial: ...`.
...
| Telnyx number webhook = `.../api/webhooks/voice` (POST) | ☐ |
| Business has Telnyx number, call screener ON, forwarding number set | ☐ |
| Test A: Press 1, answer on owner phone → no SMS | ☐ |
| Test B: Press 1, don't answer → SMS to caller | ☐ |

## Troubleshooting

- **Telnyx never hits my webhook**
  - Confirm the Telnyx webhook URL is the **ngrok HTTPS** URL (not localhost).
  - Restart ngrok and update both `.env` and Telnyx if the URL changed.

- **Dial status callback not called / wrong URL**
  - `voice-gather` builds the dial `action` URL from `NEXT_PUBLIC_APP_URL` (or `VERCEL_URL`).
  - Ensure that env is set to the same base URL Telnyx can reach (e.g. ngrok HTTPS).
...
- **No SMS when owner doesn't answer**
  ...
  - Confirm the business has `telnyxPhoneNumber` set and that the Telnyx API key in `.env` is correct.
...
- **"Business not found" in dial-status**
  ...
  - Ensure the business in the admin is the same one used when the call came in (same Telnyx number).
